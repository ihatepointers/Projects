...
with open("markovWindowSize2oaResult.txt", 'a', encoding="utf8") as markovResultFile:
                    ...
                    if((str(tmpArray[maskedIndex - 2]),str(tmpArray[maskedIndex - 1])) in myDict):
                        if(maskedIndex < len(tmpArray)-1):
                            if((key2,str(tmpArray[maskedIndex + 1])) in myDict):
                                ...
                                bestGuessChance = int(myDict[str(tmpArray[maskedIndex - 2]),str(tmpArray[maskedIndex - 1])]) * int(myDict[key1, key2]) * int(myDict[key2, str(tmpArray[maskedIndex+1])])
