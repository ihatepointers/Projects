...
with open("markovWindowSize3Result.txt", 'a', encoding="utf8") as markovResultFile:
    for line in contentData:
        ...
        for key1, key2 in myDict:
            if (int(key1) == tmpArray[maskedIndex - 1]):
                if (maskedIndex > 2):
                    if ( ((str(tmpArray[maskedIndex - 3]), str(tmpArray[maskedIndex - 2])) in myDict) and ((str(tmpArray[maskedIndex - 2]), str(tmpArray[maskedIndex - 1])) in myDict)):
                        bestGuess = contentVocab[int(key2)]
                        bestGuessChance = int(myDict[str(tmpArray[maskedIndex - 2]), str(tmpArray[maskedIndex - 1])]) * int(myDict[key1, key2]) * int(myDict[str(tmpArray[maskedIndex - 3]), str(tmpArray[maskedIndex - 2])])
                    ...
                    if((str(tmpArray[maskedIndex - 2]),str(tmpArray[maskedIndex - 1])) in myDict):
                        bestGuess = contentVocab[int(key2)]
                        bestGuessChance = int(myDict[str(tmpArray[maskedIndex - 2]),str(tmpArray[maskedIndex - 1])]) * int(myDict[key1, key2])
                        guessArray.append([bestGuess, bestGuessChance])
                    else:
                        bestGuess = contentVocab[int(key2)]
                        bestGuessChance = int(myDict[key1, key2])
